...
import { addBooking, deleteBooking, getBooking, getBookingById, updateBooking } from './database.js';
const app = express();
...
app.get('/get-booking', async (req, res) => {
    ...
    result ? setSuccessResponse(res, result, 'success') : setFailureResponse(res, result, 'No Booking Found');
})
...
app.post('/add-booking', async (req, res) => {
    ...
    if (name && date && hours) {
        ...
        pass ? setSuccessResponse(res, data, message) : setFailureResponse(res, data, message);
    } else {
...
app.post('/update-booking', async (req, res) => {
    ...
    if (id && name && date && hours) {
        ...
        result ? setSuccessResponse(res, result, 'success') : setFailureResponse(res, result, 'Somthing Went Wrong');
    } else {
        setFailureResponse(res, {}, 'Somthing Went Wrong');
    }
})
...
app.delete('/delete-booking/:id', async (req, res) => {
    if (req.params.id) {
        ...
        result ? setSuccessResponse(res, result, 'success') : setFailureResponse(res, result, 'Booking Not Cancelled');
    }
})

app.use((err, req, res, next) => {
    console.error(err.stack);
    res.status(500).send({ returnCode: 500, data: {}, message: 'Something broke!' })
})
